...
// BK - Values Updated
#define M_TO_KM 1.47695518286
#define MSUN_DENS_TO_CGI 6.17327129839e17 // Convert rest-mass density to CGI for M=Msun
...
double SphericalHydroSource::getRhohat(double xx, double yy, double zz)
{
	DECLARE_CCTK_PARAMETERS;
	double rr = getRelativeDistance(xx, yy, zz);
	double rhohat = std::pow(1.+getNonFlatPsiOfR(rr), rhohat_exponent)*((getRho(rr)+getPress(rr))*getWW2(xx, yy, zz) - getPress(rr));
...
TOVStar::TOVStar(int idx) : SphericalHydroSource(idx)
{
	...
	if(testsuite)
	{
		otable << "#1:riso" << "2:press" << "3:psi" << "4:theta" << "5:qq" << "6:cc" << "7::nn";
		otable << std::endl;
	}
    ...
    for(int l = 0; l < inside_points; l++)
    {
		...
		if(testsuite)
		{
			otable << rk4data[l].riso << rk4data[l].press << rk4data[l].psi0 << rk4data[l].theta0 << rk4data[l].qq << rk4data[l].cc << rk4data[l].nn;
			otable << std::endl;
		}
    }
